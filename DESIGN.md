...
| 2026-06-08 | Chain logos in selectors + explorer (user-requested) | Recognising "the type of crypto" at a glance needs the real brand mark, so multicolor logos are allowed despite the restrained palette. Contained in 20px circular chips (full radius — DESIGN.md's chain-selector radius) so they read as controlled tokens, not decoration. Native `<option>` can't render images, so the chain selector is now a progressive-enhancement custom listbox over a hidden native `<select>` (the select stays the value source of truth). Logos load from a CDN; any miss falls back to an on-brand neutral monogram chip — never a broken image. |
| 2026-06-08 | Standing "unattended / no liquidity" notice banner | A persistent amber `.notice-banner` at the top of all three apps states plainly that the system works but no one is monitoring it or providing liquidity, and that a user must bring a counterparty on the destination chain (who runs the Solver app). Honesty over polish — the trust thesis means not implying a live market that isn't there. Amber (warning token) reads as caution, contained like the `.trust` callout; copy is tailored per app (user / solver / explorer). No JS, no dismiss — it's a standing condition, not a transient alert. |
| 2026-06-08 | Ship only chains with a proven settlement path | The chain selector lists 24 EVM mainnets (+ testnets), not the full top-30. Six chains (zkSync Era, Arbitrum, Aurora, Kava, Metis, Polygon zkEVM) are excluded until a live funded settlement proves them — surfacing an unverified chain in a money-moving dropdown is a trust failure, and trust is this product's hero. The engine already supports what they need (legacy tx + per-chain gas); they're held in code/config only, with the re-enable path in TODOS.md. Rationale + per-chain detail there. |
